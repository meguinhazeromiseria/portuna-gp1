...
class SodreExtractor:
    ...
    def extrair(self):
        ...
        for pag in range(15):
            ...
            eletro = [l for l in lotes if self._is_eletro(l)]
            items.extend(eletro)
            
            print(f"  Pág {pag+1}: +{len(eletro)} | Total: {len(items)}")
    ...
    def _normalizar(self, items):
        return [{
            "source": "sodre",
            "external_id": f"sodre_eletro_{i.get('lot_id')}",
...
class SuperbidExtractor:
    ...
    def extrair(self):
        ...
        for pag in range(1, 15):
            params = {
                "urlSeo": f"{self.BASE}/categorias/eletrodomesticos",
                "pageNumber": pag,
                "pageSize": 100,
            ...
            for o in offers:
                if o.get("store", {}).get("name"):
                    items.append({
                        "source": "superbid",
                        "external_id": f"superbid_eletro_{o.get('id')}",
